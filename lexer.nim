...
type
  TokenKind* = enum
    tkLpar, tkRpar, tkDot, tkQuote, tkSym, tkNum, tkStr, tkEof
...
proc parseNumber*(lx: var AshLexer, start: int) =
  ...
  if pos < lx.buf.len and lx.buf[pos] in {'e', 'E'}:
    inc pos
    if pos < lx.buf.len and lx.buf[pos] in {'+', '-'}: inc pos
    while pos < lx.buf.len and lx.buf[pos].isDigit: inc pos
    
  let numStr = lx.buf.substr(start, pos - 1)
  lx.bufpos = pos
  try:
    lx.curTok = Token(kind: tkNum, num: parseFloat(numStr))
  except:
